...
numerical_columns = c('calories','protein','fat','sodium','fibre','carbo','sugars','potassium')
...
categorical_columns = c('mfr','shelf')
...
plot(data$fibre, data$potassium,
     xlab = 'fibre', ylab = 'potassium', 
     main = "Scatter Plot of potassium vs fibre",pch=19, col='lightblue')
reg_line = lm(potassium ~ fibre, data=data)
abline(reg_line, col = "red", lwd = 2)
intercept = coef(reg_line)[1]
slope = coef(reg_line)[2]
equation_text = paste("y = ", round(slope, 2), "x +", round(intercept, 2))
text(x = max(data$fibre) * 0.8,
     y = max(data$potassium) * 0.9,
     labels = equation_text, col = 'red', cex = 0.8)
print(paste("equation of line:",equation_text,""))
print(summary(reg_line))
qqnorm(reg_line$residuals)
...
## Multiple regression----
par(mfrow=c(2,2))
mult_reg = c('calories','protein','fat','sodium','fibre','carbo')
mult_reg_data = data[,mult_reg]
fit_mult = lm(calories~protein+fat+sodium+fibre+carbo,data=mult_reg_data)
summary(fit_mult)
plot(fit_mult)
...
# ADVANCED ANLYTICS----
pca_cols=c('protein','fat','sodium','fibre','sugars','potassium','carbo')
